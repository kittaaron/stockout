...
def calc_pe_eps(code, name, stock):
    ...
    logging.info("%s %s 流动资产:%s 总负债:%s 流动负债: flow_debts, 总市值:%s", code, name, flow_assets, total_debts, flow_debts, mktcap)
    old_safe = session.query(Safe).filter(and_(Safe.code == code)).first()
    if old_safe is None:
        old_safe = Safe(code = code, name = name)
    old_safe.result = round((flow_assets - total_debts) / mktcap, 2)
    old_safe.result2 = round((flow_assets - flow_debts) / mktcap, 2)
